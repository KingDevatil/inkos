...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  const clearStoreModels = useServiceStore((s) => s.clearStoreModels);
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchJson<{ services: Array<Record<string, unknown>> }>("/v1/services/config")
      .then((data) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    if (svc?.connected) {
      ...
      fetchJson<{ models: ModelInfo[] }>(`/v1/services/${encodeURIComponent(serviceId)}/models`)
        .then((data) => {
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchJson<{ apiKey?: string }>(`/v1/services/${encodeURIComponent(effectiveServiceId)}/secret`)
      .then((data) => {
        if (cancelled) return;
  ...
  const handleTest = async () => {
    ...
    try {
      const result = await fetchJson<{ ok: boolean; models?: ModelInfo[]; modelCount?: number; error?: string }>(
        `/v1/services/${encodeURIComponent(effectiveServiceId)}/test`,
        {
          method: "POST",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await fetchJson(`/v1/services/${encodeURIComponent(effectiveServiceId)}/secret`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ apiKey: trimmedKey }),
      });
      // Save config (temperature, maxTokens, etc.)
      await fetchJson("/v1/services/config", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          services: [
      ...
      if (trimmedKey) {
        try {
          const data = await fetchJson<{ models: ModelInfo[] }>(`/v1/services/${encodeURIComponent(effectiveServiceId)}/models`);
          const m = data.models ?? [];
          if (m.length > 0) {
