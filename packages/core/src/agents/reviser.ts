...
const MODE_DESCRIPTIONS: Record<ReviseMode, string> = {
  ...
  rewrite: "改写：基于当前设定，允许重组问题段落、调整画面和叙述力度，但优先保留原文的绝大部分句段。除非问题跨越整章，否则禁止整章推倒重写；只能围绕问题段落及其直接上下文改写，同时保留核心事实与人物动机",
  rework: "重写：基于当前设定，可大幅重构场景推进和冲突组织，让章节适应新大纲，但保持当前主设定和大事件结果不变",
...
export class ReviserAgent extends BaseAgent {
  ...
  async reviseChapter(
    ...
    const issueList = sortedIssues
      .map((i, idx) => `${idx + 1}. [${i.severity.toUpperCase()}] ${i.category}: ${i.description.slice(0, 200)}${i.description.length > 200 ? '...' : ''}\n   修复建议: ${i.suggestion.slice(0, 150)}${i.suggestion.length > 150 ? '...' : ''}`)
      .join("\n\n");
    
    // 记录实际传递的问题数量
    console.log(`[ReviserAgent] 传递 ${sortedIssues.length} 个审稿问题给 LLM:`);
    sortedIssues.forEach((i, idx) => {
      console.log(`  ${idx + 1}. [${i.severity}] ${i.category}`);
    });
...
(更新后的完整伏笔池)`;
    ...
    // 构建审稿问题摘要，放在systemPrompt开头以强调重要性
    ...
    const infoIssues = sortedIssues.filter(i => i.severity === 'info');
    
    const issueSummary = sortedIssues.length > 0 
      ? `\n\n【审稿问题摘要】共${sortedIssues.length}个问题待修复：\n` +
        (criticalIssues.length > 0 ? `- 严重问题：${criticalIssues.length}个（必须优先处理）\n` : '') +
        (warningIssues.length > 0 ? `- 警告问题：${warningIssues.length}个\n` : '') +
        (infoIssues.length > 0 ? `- 提示问题：${infoIssues.length}个\n` : '') +
        `\n详细问题列表见用户消息中的"审稿问题"部分。`
      : '';

    const systemPrompt = `${langPrefix}你是一位专业的${gp.name}网络小说修稿编辑。你的任务是根据审稿意见对章节进行修正。${protagonistBlock}${issueSummary}

修稿模式：${modeDesc}

【核心任务 - 按优先级执行】
1. **首先处理所有严重(critical)审稿问题** - 这些问题阻碍章节通过审核
2. **然后处理警告(warning)问题** - 这些问题影响质量
3. **最后处理提示(info)问题** - 这些问题可以优化体验
4. 在 FIXED_ISSUES 中必须列出所有已修复的问题，不得遗漏
5. 如果某个问题无法修复，必须在 FIXED_ISSUES 中说明原因
...
重要：作者意图是对审稿问题的补充，请同时处理审稿问题和作者意图。`
      : "";

    // 构建按严重程度分组的问题列表
    const buildCategorizedIssueList = () => {
      if (sortedIssues.length === 0) return "（无审稿问题）";
      
      const criticalList = sortedIssues.filter(i => i.severity === 'critical');
      const warningList = sortedIssues.filter(i => i.severity === 'warning');
      const infoList = sortedIssues.filter(i => i.severity === 'info');
      ...
      if (criticalList.length > 0) {
        result += `\n\n【严重问题 - 必须优先修复】\n`;
        result += criticalList.map(i => 
          `${idx++}. ${i.category}: ${i.description.slice(0, 250)}${i.description.length > 250 ? '...' : ''}\n   修复建议: ${i.suggestion.slice(0, 200)}${i.suggestion.length > 200 ? '...' : ''}`
        ).join('\n\n');
      }
      
      if (warningList.length > 0) {
        result += `\n\n【警告问题 - 需要修复】\n`;
        result += warningList.map(i => 
          `${idx++}. ${i.category}: ${i.description.slice(0, 250)}${i.description.length > 250 ? '...' : ''}\n   修复建议: ${i.suggestion.slice(0, 200)}${i.suggestion.length > 200 ? '...' : ''}`
        ).join('\n\n');
      }
    ...
    const categorizedIssueList = buildCategorizedIssueList();
...
【处理要求】
- 严重问题：必须修复，否则章节无法通过审核
- 警告问题：应当修复，影响章节质量
- 提示问题：建议修复，可优化阅读体验
- 在 FIXED_ISSUES 中列出所有已修复的问题
- 如果某个问题无法修复，说明原因
${briefBlock}
...
${chapterContent}`;
  ...
  private parseOutput(
    ...
    const extract = (tag: string): string => {
      ...
      console.log(`[ReviserAgent.parseOutput] Extracting ${tag}: ${result ? "found" : "NOT FOUND"} (${result.length} chars)`);
      return result;
    };
    ...
    if (mode === "spot-fix") {
      ...
      console.log(`[ReviserAgent.parseOutput] spot-fix mode: applied=${patchResult.applied}, revisedContent length=${patchResult.revisedContent.length}`);
    ...
    console.log(`[ReviserAgent.parseOutput] ${mode} mode: revisedContent length=${revisedContent.length}, fixedIssues count=${fixedIssues.length}`);

    // 如果提取的修订内容为空，可能是LLM没有按照格式返回，记录原始响应的前500字符用于调试
    if (revisedContent.length === 0) {
      console.warn(`[ReviserAgent.parseOutput] WARNING: REVISED_CONTENT is empty! Raw response preview: ${content.slice(0, 500)}...`);
    }
